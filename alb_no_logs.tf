resource "aws_lb" "application_no_logs" {
  ...
  count = "${local.enabled_no_log ? 1 : 0}"
}
...
resource "aws_lb_target_group" "main_no_logs" {
  ...
  count      = "${local.enabled_no_log ? var.target_groups_count : 0}"
...
resource "aws_lb_listener" "frontend_http_tcp_no_logs" {
  ...
  count             = "${local.enabled_no_log ? var.http_tcp_listeners_count : 0}"
...
resource "aws_lb_listener" "frontend_https_no_logs" {
  ...
  count             = "${local.enabled_no_log ? var.https_listeners_count : 0}"
...
resource "aws_lb_listener_certificate" "https_listener_no_logs" {
  ...
  count           = "${local.enabled_no_log ? var.extra_ssl_certs_count : 0}"
}
