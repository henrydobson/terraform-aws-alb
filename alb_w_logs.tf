...
resource "aws_lb_target_group" "main" {
  ...
  count      = "${local.enabled_w_log ? var.target_groups_count : 0}"
...
resource "aws_lb_listener" "frontend_http_tcp" {
  ...
  count             = "${local.enabled_w_log ? var.http_tcp_listeners_count : 0}"
...
resource "aws_lb_listener" "frontend_https" {
  ...
  count             = "${local.enabled_w_log ? var.https_listeners_count : 0}"
...
resource "aws_lb_listener_certificate" "https_listener" {
  ...
  count           = "${local.enabled_w_log ? var.extra_ssl_certs_count : 0}"
}
