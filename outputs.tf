...
output "http_tcp_listener_arns" {
  ...
  value       = "${concat(aws_lb_listener.frontend_http_tcp.*.arn, aws_lb_listener.frontend_http_tcp_no_logs.*.arn)}"
}

output "http_tcp_listener_ids" {
  description = "The IDs of the TCP and HTTP load balancer listeners created."
  value       = "${concat(aws_lb_listener.frontend_http_tcp.*.id, aws_lb_listener.frontend_http_tcp_no_logs.*.id)}"
}

output "https_listener_arns" {
  description = "The ARNs of the HTTPS load balancer listeners created."
  value       = "${concat(aws_lb_listener.frontend_https.*.arn, aws_lb_listener.frontend_https_no_logs.*.arn)}"
}

output "https_listener_ids" {
  description = "The IDs of the load balancer listeners created."
  value       = "${concat(aws_lb_listener.frontend_https.*.id, aws_lb_listener.frontend_https_no_logs.*.id)}"
}
